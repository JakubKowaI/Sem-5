...
int main() {
    ...
    const int PER_PROCESS_TIMEOUT_SEC = 300;     // limit czasu dla procesu w sekundach (30s as requested)

    // MIN_FREE_MB = 0 -> wyłącz kontrolę pamięci; wtedy jedyną barierą jest MAX_CONCURRENT.

    vector<string> families = {"USA-road-d"};
    vector<string> limits = {"BAY","CAL","COL","CTR","E","FLA","LKS","NE","NY","W"};
    ...
    for (size_t i = 0; i < families.size(); ++i) {
        ...
        for (string test:limits) {
            string base = "ch9-1.1/inputs/" + family + "/" + family + "." + test;
            ...
            ofstream single("processed/"+family + "." + test + ".single.ss");
            ofstream random("processed/"+family + "." + test + ".random.ss");
            ofstream p2pout("processed/"+family + "." + test + ".p2p");
    ...
    for (size_t i = 0; i < families.size(); ++i) {
        ...
        for (string test : limits) {
            string base = "ch9-1.1/inputs/" + family + "/" + family + "." + test;
            string diala_cmd_ss_single    = "./diala    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".single.ss" + " -oss wyniki/diala/"    + family + "." + test + ".single.ss.res";
            string diala_cmd_ss_random    = "./diala    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".random.ss" + " -oss wyniki/diala/"    + family + "." + test + ".random.ss.res";
            string diala_cmd_p2p    = "./diala    -d " + base + ".gr -p2p " + "processed/"+family + "." + test + ".p2p" + " -op2p wyniki/diala/"    + family + "." + test + ".p2p.res";

            string dijkstry_cmd_ss_single    = "./dijkstra    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".single.ss" + " -oss wyniki/dijkstra/"    + family + "." + test + ".single.ss.res";
            string dijkstry_cmd_ss_random    = "./dijkstra    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".random.ss" + " -oss wyniki/dijkstra/"    + family + "." + test + ".random.ss.res";
            string dijkstry_cmd_p2p    = "./dijkstra    -d " + base + ".gr -p2p " + "processed/"+family + "." + test + ".p2p" + " -op2p wyniki/dijkstra/"    + family + "." + test + ".p2p.res";

            string radix_cmd_ss_single    = "./radixheap    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".single.ss" + " -oss wyniki/radixheap/"    + family + "." + test + ".single.ss.res";
            string radix_cmd_ss_random    = "./radixheap    -d " + base + ".gr -ss " + "processed/"+family + "." + test + ".random.ss" + " -oss wyniki/radixheap/"    + family + "." + test + ".random.ss.res";
            string radix_cmd_p2p    = "./radixheap    -d " + base + ".gr -p2p " + "processed/"+family + "." + test + ".p2p" + " -op2p wyniki/radixheap/"    + family + "." + test + ".p2p.res";

            pool.enqueue(diala_cmd_ss_single);
            pool.enqueue(diala_cmd_ss_random);
            pool.enqueue(diala_cmd_p2p);

            pool.enqueue(dijkstry_cmd_ss_single);
            pool.enqueue(dijkstry_cmd_ss_random);
            pool.enqueue(dijkstry_cmd_p2p);
